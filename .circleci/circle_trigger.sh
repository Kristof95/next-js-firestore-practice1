...
if [[ ${LAST_COMPLETED_BUILD_SHA} == "null" ]]; then
  ...
  LAST_COMPLETED_BUILD_SHA=feature/asd-123
fi
...
PARAMETERS='"trigger":false, "initialize":false'
COUNT=0
...
HTTP_RESPONSE=$(curl -vvv -s -u "${CIRCLE_TOKEN}": -o response.txt -w "%{http_code}" -X POST --header "Content-Type: application/json" -d "$DATA" $URL)
echo "URL: ${URL}"
